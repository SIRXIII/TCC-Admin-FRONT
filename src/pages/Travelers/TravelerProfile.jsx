...
const TravelerProfile = () => {
  ...
  const filteredOrders = traveler.order.filter(
    (order) =>
      order.id.toString().includes(searchTerm) ||
      order.partner_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      order.status.toLowerCase().includes(searchTerm.toLowerCase())
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="flex flex-col gap-6 flex-1">
          <div
            ...
            style={{ boxShadow: "0px 0px 3px 0px #00000033" }}
          >
            <h2 className="text-lg fw6 text-[#232323]">Traveler Information</h2>
            <div className="flex items-center gap-4 py-2.5">
              <img
                src={traveler.profile_photo}
                alt="Traveler"
                className="w-14 h-14 rounded-xl object-cover object-center cursor-pointer"
                onClick={() => setPreviewImage(traveler.profile_photo || DefaultProfile)}
                onError={(e) => {
                  e.currentTarget.src = DefaultProfile;
                }}
              />
              <div>
                <h3 className="text-lg fw6 text-[#232323]">{traveler.name}</h3>
                <p className="text-xs text-[#6C6C6C]">
                  <a href={`mailto:${traveler?.email}`} className="hover:underline">
                    {traveler?.email}
                  </a>
            ...
            <div className="grid grid-cols-2">
              <div className="flex flex-col gap-2">
                <p className="text-sm fw6">Last Order</p>
                <p className="fw5 text-xs text-[#9A9A9A]">
                  {daysAgo} <span className="text-[#F77F00]">{lastOrderId ?? "#" + lastOrderId}</span>
                </p>
              </div>
              <div className="flex flex-col gap-2">
                <p className="text-sm fw6">Total Spent</p>
